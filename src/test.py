...
-----------------------------------------------------------------------------------
# Description: Testing script (Modified for Headless Server)
"""
...
# ====== 【修改1】服务器防崩溃补丁 (强制无头模式) ======
import matplotlib
matplotlib.use('Agg')  # 强制不使用图形界面后端

import cv2
# 暴力屏蔽 cv2 的显示函数，防止在服务器上报错
def no_op(*args, **kwargs): pass
cv2.imshow = no_op
cv2.namedWindow = no_op
...
def parse_test_configs():
    ...
    # ====== 【修改2】参数默认值调整 ======
    # show_image 保持默认 False，防止弹窗
    parser.add_argument('--show_image', action='store_true',
                        help='If true, show the image during demostration')
    # save_test_output 建议默认逻辑在下面强制覆盖，这里保留参数定义
    ...
    # ====== 【修改3】强制开启保存，强制关闭显示 ======
    configs.save_test_output = True  # 强制保存结果
    configs.show_image = False       # 强制不显示 (虽然前面有补丁，双重保险)
    # ============================================
...
if __name__ == '__main__':
    ...
    # 检查权重文件
    if configs.pretrained_path is None:
        print("[警告] 您没有指定权重文件路径 (--pretrained_path)。")
        print("请在运行命令中加入: --pretrained_path checkpoints/complex_yolov4/best.pth (举例)")
        # 为了防止直接报错退出，这里做个简单的检查，或者您可以手动硬编码路径：
        # configs.pretrained_path = 'checkpoints/complex_yolov4/best.pth' 
    
    if configs.pretrained_path and os.path.isfile(configs.pretrained_path):
        model.load_state_dict(torch.load(configs.pretrained_path, map_location=device_string))
    else:
        print(f"[Error] No checkpoint found at {configs.pretrained_path}")
        # sys.exit(1) # 如果没有权重，通常无法继续，您可以取消注释这行
    ...
    print(f"开始推理... 结果将保存在: {os.path.abspath(configs.results_dir)}")
        ...
        for batch_idx, (img_paths, imgs_bev) in enumerate(test_dataloader):
            ...
            if configs.save_test_output:
                if configs.output_format == 'image':
                    ...
                    save_path = os.path.join(configs.results_dir, '{}.jpg'.format(img_fn))
